...
export const isValidSudoku = function(board: string[][] ): boolean {
  ...
  const block: Set<string>[] = Array.from({length:9}, () => new Set());

  for(let i = 0; i < 9; i++){
    for(let j = 0; j < 9; j++){
      const num = board[i][j];
      if(num !== '.'){
        if(row[i].has(num) || col[j].has(num) || block[(i / 3 | 0) * 3 + j / 3 | 0].has(num)){
          return false;
        }
        row[i].add(num);
        col[j].add(num);
        block[(i / 3 | 0) * 3 + j / 3 | 0].add(num);
      }
    }
  }
  return true;
