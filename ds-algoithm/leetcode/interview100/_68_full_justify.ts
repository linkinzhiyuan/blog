...
/**
 * 逐词遍历，依次放入能放入的当前行中
 * 不能放入时，均匀分配空格，只有一个单词直接左对齐，右侧补空格，多个单词则尽量平均分配空格，多余的空格分配到左侧
 * 最后一行单词左对齐，填充空格
 * @param {string[]} words
 * @param {number} maxWidth
 * @return {string[]}
 */
export const fullJustify = function(words: string[], maxWidth: number): string[] {
  let res: string[] = [];
    let line: string[] = [];
    let lineLength = 0;
    ...
    for (let word of words) {
        ...
        if (lineLength + word.length + line.length > maxWidth) {
            ...
            for (let i = 0; i < spacesToDistribute; i++) {
                line[i % (line.length - 1 || 1)] += ' ';  // 将空格均匀分布到单词之间
            }
            res.push(line.join(''));
            line = [];
            lineLength = 0;
